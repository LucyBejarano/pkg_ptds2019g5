shiny::shinyServer(function(input, output) {
  ...
  output$Map_NO2 <- plotly::renderPlotly({
    plotly::ggplotly(
      pollutant_month_avg %>% 
        ...
        ggplot2::geom_point(aes(x = longitude, y = latitude, frame = time, color = pollutant_month_avg, size = pollutant_month_avg, tooltip = name)) +
  ...
  output$calendar_precipitation <- 
    shiny::renderPlot({
      ...
      weather_df$date <- as.Date(format(wide_df$Date_time, "%Y-%m-%d"))
